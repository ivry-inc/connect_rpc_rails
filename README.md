...
caller ──HTTP──▶ Rails router ──▶ GreetController#say_hello ──▶ handler (PORO)
                                  (ConnectRpcRails::Controller: decode ▸ interceptors ▸ encode)
```
...
Rails.application.routes.draw do
  ConnectRpcRails::Routing.mount(self, GreetController)
end
```
...
connect_service Greet::V1::SERVICE_DESCRIPTOR,
  handler: Greet::V1::GreetHandler.new,
  interceptors: [
    ConnectRpcRails::ExceptionMappingInterceptor.new(
...
examples/greet/           # the example wired up as a real Rails app tree
  ...
  config/routes.rb                       # ConnectRpcRails::Routing.mount(self, ...)
